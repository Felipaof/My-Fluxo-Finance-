...
async function marcarMetaConcluida(id) {
  ...
  if (confirmacao) {
    try {
      await api.updateMeta(id, { foi_batida: true });
      showNotification('🎉 Parabéns! Meta concluída com sucesso!', 'success');
      ...
      showNotification(error.message || 'Erro ao marcar meta como concluída', 'error');
    }
  }
}
...
function validarFormulario() {
  ...
  if (!dataFinal) {
    showFieldError('data_final', 'Data final é obrigatória');
    isValid = false;
  }
  
  if (dataInicio && dataFinal && new Date(dataFinal) <= new Date(dataInicio)) {
    showFieldError('data_final', 'Data final deve ser maior que data inicial');
    isValid = false;
  }
